...
console.log(`Applying signage contact schema to project: ${projectRef}`);
const result = await post(`/v1/projects/${projectRef}/database/query`, { query: sql }, accessToken);

if (result.status === 200 || result.status === 201) {
  console.log('Signage contact schema applied.');
} else {
  console.error(`Signage contact schema failed (HTTP ${result.status}):`);
