...
const statusStyles: Record<Project['status'], { color: string; label: string }> = {
  on_schedule: { color: '#38bdf8', label: 'On Schedule' },
                          ...
                          {branchProjects.length > 0 ? (
                            branchProjects.map((project) => (
                              <Link
                                ...
                                className="inline-flex items-center justify-center rounded-md border border-slate-600 bg-slate-700 px-2 py-1 text-[11px] font-semibold uppercase tracking-wide text-slate-100 transition hover:bg-slate-600"
                              >
              ...
              <Marker key={project.id} position={position} icon={createProjectIcon(project)}>
                <Popup>
                  <div className="min-w-48 text-slate-900">
                    ...
                    <Link className="mt-3 inline-flex items-center rounded-lg border border-emerald-300/30 bg-emerald-600 px-3 py-2 text-xs font-semibold text-white transition hover:bg-emerald-500" to={`/branches/${encodeURIComponent(project.branchId ?? project.branch)}`}>
                      View project details
                    </Link>
                  </div>
                </Popup>
